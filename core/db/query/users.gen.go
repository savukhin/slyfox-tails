...
func newUser(db *gorm.DB, opts ...gen.DOOption) user {
	...
	_user.Projects = userHasManyProjects{
		...
		Jobs: struct {
			...
			RelationField: field.NewRelation("Projects.Jobs", "models.Job"),
			Stages: struct {
				field.RelationField
			}{
				RelationField: field.NewRelation("Projects.Jobs.Stages", "models.Stage"),
			},
		},
	}
...
func (a userHasManyPoints) Where(conds ...field.Expr) *userHasManyPoints {
	if len(conds) == 0 {
		return &a
	}
...
func (a userHasManyPoints) WithContext(ctx context.Context) *userHasManyPoints {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a userHasManyPoints) Session(session *gorm.Session) *userHasManyPoints {
	a.db = a.db.Session(session)
	return &a
}

func (a userHasManyPoints) Model(m *models.User) *userHasManyPointsTx {
	return &userHasManyPointsTx{a.db.Model(m).Association(a.Name())}
}

type userHasManyPointsTx struct{ tx *gorm.Association }

func (a userHasManyPointsTx) Find() (result []*models.Point, err error) {
	return result, a.tx.Find(&result)
}

func (a userHasManyPointsTx) Append(values ...*models.Point) (err error) {
...
func (a userHasManyPointsTx) Replace(values ...*models.Point) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a userHasManyPointsTx) Delete(values ...*models.Point) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a userHasManyPointsTx) Clear() error {
	return a.tx.Clear()
}

func (a userHasManyPointsTx) Count() int64 {
	return a.tx.Count()
}
...
func (a userHasManyProjects) Model(m *models.User) *userHasManyProjectsTx {
	return &userHasManyProjectsTx{a.db.Model(m).Association(a.Name())}
}

type userHasManyProjectsTx struct{ tx *gorm.Association }

func (a userHasManyProjectsTx) Find() (result []*models.Project, err error) {
	return result, a.tx.Find(&result)
}

func (a userHasManyProjectsTx) Append(values ...*models.Project) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a userHasManyProjectsTx) Replace(values ...*models.Project) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a userHasManyProjectsTx) Delete(values ...*models.Project) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
